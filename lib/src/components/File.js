...
exports.FileExplorer = exports.FileNavigator = exports.PathNavigator = exports.FilesIconView = exports.FilesListView = exports.FilesView = void 0;
...
function _readDir(path, callback, errorCallback) {
    if (_dirs[path] != undefined) {
        callback(path);
    }
    else {
        File_1.File.readDir(path, callback, dir);
    }
}
/**
 * That class is the base class of FilesListView and FilesIconView
 */
class FilesView extends HTMLElement {
    constructor() {
        super();
        // Set the shadow dom.
        ...
        Model_1.Model.eventHub.subscribe("set_dir_event", (uuid) => {
            /** Nothin here. */
        }, (dir) => {
            this.setDir(dir);
        });
 ...
 */
class FilesListView extends FilesView {
    constructor() {
        super();
    }
    /**
    ...
    setDir(dir) {
        ...
        for (let f of dir.files) {
                ...
                if (f.size > 1024) {
                    if (f.size > 1024 * 1024) {
                        if (f.size > 1024 * 1024 * 1024) {
                            ...
                            size = fileSize.toFixed(0) + " Gb";
                        }
                        else {
                            let fileSize = f.size / (1024 * 1024);
                            size = fileSize.toFixed(0) + " Mb";
                        }
                    }
                    else {
                        let fileSize = f.size / 1024;
                        size = fileSize.toFixed(0) + " Kb";
                    }
                }
                else {
                    size = f.size + " bytes";
            ...
            if (f.isDir) {
                ...
                lnk.onclick = () => {
                    Model_1.Model.eventHub.publish("set_dir_event", f, true);
                };
                lnk.onmouseover = () => {
    ...
    setDir(dir) {
        ...
        for (const dir of values) {
                        ...
                        for (let subDir of dir.files) {
                            ...
                            if (subDir.isDir) {
                                ...
                                subDirSpan.onclick = (evt) => {
                                    evt.stopPropagation();
                                    Model_1.Model.eventHub.publish("set_dir_event", subDir, true);
                                    directoriesDiv.style.display = "none";
                                    btn.icon = "icons:chevron-right";
                                };
                            }
            ...
            title.onclick = () => {
                _readDir(path_, (dir) => {
                    // Send read dir event.
                    Model_1.Model.eventHub.publish("set_dir_event", dir, true);
                }, this.onerror);
            };
...
class FileExplorer extends HTMLElement {
    ...
    init() {
        if (this.path == undefined) {
            ...
            this.path = "/" + Model_1.Model.application; // set to the application folder.
        }
        // Read the fd
        ...
        <paper-card id="file-explorer-box" class="file-explorer" style="flex-direction: column; display: none;">
            <div id="file-explorer-content" class="card-content">
                <div style="display: flex; align-items: center;">
                    ...
                    <paper-icon-button id="navigation-foward-btn" icon="icons:arrow-upward"></paper-icon-button>
                ...
                <div style="display: flex; flex-grow: 1;">
                    ...
                    <div  id="file-selection-panel" style="position: relative;">
                        ...
                        <div style="position: absolute; bottom: 0px; right: 0px; display: flex; background-color:var(--palette-background-default)" >
