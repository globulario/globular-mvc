    ...
    setDir(dir) {
        ...
        for (let f of dir.files) {
            ...
            if (f.isDir) {
                ...
                lnk.onclick = () => {
                    Model_1.Model.eventHub.publish("set_dir_event", _dirs[f._path], true);
                };
                lnk.onmouseover = () => {
    ...
    setDir(dir) {
        ...
        for (const dir of values) {
                        ...
                        for (let subDir of dir.files) {
                            ...
                            if (subDir.isDir) {
                                ...
                                subDirSpan.onclick = (evt) => {
                                    evt.stopPropagation();
                                    Model_1.Model.eventHub.publish("set_dir_event", _dirs[subDir._path], true);
                                    directoriesDiv.style.display = "none";
                                    btn.icon = "icons:chevron-right";
                                };
                            }
            ...
            title.onclick = () => {
                _readDir(path_, (dir) => {
                    // Send read dir event.
                    Model_1.Model.eventHub.publish("set_dir_event", _dirs[dir._path], true);
                }, this.onerror);
            };
...
class FileExplorer extends HTMLElement {
    ...
    init() {
        ...
        if (this.root == undefined) {
            ...
            this.root = "/" + Model_1.Model.application; // set to the application folder.
        }
        // Read the fd
        ...
        if (this.navigations.length > 2) {
            ...
            for (let path of this.navigations) {
                ...
                navigationLine.onclick = () => {
                    navigationLst.style.display = "none";
                    Model_1.Model.eventHub.publish("set_dir_event", _dirs[this.navigations[index]], true);
                };
            }
    ...
    connectedCallback() {
        ...
        this.upwardNavigationBtn.onclick = () => {
            if (this.path.split("/").length > 2) {
                ...
                Model_1.Model.eventHub.publish("set_dir_event", _dirs[this.path], true);
            }
        };
